...
export function setTokenCookie(name: string, token: string) {
  cookies().set({
    ...
    httpOnly: false,
    secure: true,
    maxAge: getTokenExpiration(token) - Math.floor(Date.now() / 1000),
    sameSite: 'lax',
  })
}
...
export async function checkToken() {
  ...
  // アクセストークンをクッキーにセット
  setTokenCookie(ACCESS_TOKEN_NAME, data.accessToken)

  // リフレッシュトークンをクッキーにセット
  setTokenCookie(REFRESH_TOKEN_NAME, data.refreshToken)
